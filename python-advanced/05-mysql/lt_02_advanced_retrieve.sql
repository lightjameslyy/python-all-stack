...
-- 自关联
	...
	-- 查询出山东省有哪些市
    select province.name, city.name from region as province inner join region as city on city.parentCode=province.code having province.name="山东";

	-- 查询出青岛市有哪些县城
    select province.name, city.name from region as province inner join region as city on city.parentCode=province.code having province.name="青岛";
    select * from region where parentCode = (select code from region where name="青岛");
